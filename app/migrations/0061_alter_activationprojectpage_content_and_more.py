# Generated by Django 4.1.3 on 2023-04-03 22:46

import wagtail.blocks
import wagtail.fields
import wagtail.images.blocks
from django.db import migrations


class Migration(migrations.Migration):

    dependencies = [
        ("app", "0060_fix_user_page_field"),
    ]

    operations = [
        migrations.AlterField(
            model_name="activationprojectpage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="countrypage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="eventpage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="homepage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="impactareapage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="landingpage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="openmappinghubindexpage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="opportunitypage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="organisationpage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="personpage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="toolpage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="topichomepage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
        migrations.AlterField(
            model_name="topicpage",
            name="content",
            field=wagtail.fields.StreamField(
                [
                    ("richtext", wagtail.blocks.RichTextBlock(group="Basic")),
                    (
                        "image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=True
                                    ),
                                ),
                                ("caption", wagtail.blocks.CharBlock(required=False)),
                            ]
                        ),
                    ),
                    (
                        "call_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=400,
                                        required=False,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        required=False
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.StreamBlock(
                                        [
                                            (
                                                "internal_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "page",
                                                            wagtail.blocks.PageChooserBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="If set this replaces the page title",
                                                                required=False,
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                            (
                                                "external_link",
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "url",
                                                            wagtail.blocks.URLBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                        (
                                                            "label",
                                                            wagtail.blocks.CharBlock(
                                                                required=True
                                                            ),
                                                        ),
                                                    ]
                                                ),
                                            ),
                                        ],
                                        max_num=2,
                                        min_num=0,
                                        required=False,
                                    ),
                                ),
                                (
                                    "background",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[("dark", "Dark"), ("light", "Light")]
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_left", "Image left"),
                                            ("image_right", "Image right"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "gallery_of_calls_to_action",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "rows",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.ListBlock(
                                            wagtail.blocks.StructBlock(
                                                [
                                                    (
                                                        "title",
                                                        wagtail.blocks.CharBlock(
                                                            max_length=75, required=True
                                                        ),
                                                    ),
                                                    (
                                                        "description",
                                                        wagtail.blocks.RichTextBlock(
                                                            features=[
                                                                "italic",
                                                                "bold",
                                                                "link",
                                                            ],
                                                            max_length=400,
                                                            required=False,
                                                        ),
                                                    ),
                                                    (
                                                        "image",
                                                        wagtail.images.blocks.ImageChooserBlock(
                                                            required=False
                                                        ),
                                                    ),
                                                    (
                                                        "links",
                                                        wagtail.blocks.StreamBlock(
                                                            [
                                                                (
                                                                    "internal_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "page",
                                                                                wagtail.blocks.PageChooserBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    help_text="If set this replaces the page title",
                                                                                    required=False,
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                                (
                                                                    "external_link",
                                                                    wagtail.blocks.StructBlock(
                                                                        [
                                                                            (
                                                                                "url",
                                                                                wagtail.blocks.URLBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                            (
                                                                                "label",
                                                                                wagtail.blocks.CharBlock(
                                                                                    required=True
                                                                                ),
                                                                            ),
                                                                        ]
                                                                    ),
                                                                ),
                                                            ],
                                                            max_num=2,
                                                            min_num=0,
                                                            required=False,
                                                        ),
                                                    ),
                                                ]
                                            ),
                                            max_num=3,
                                            min_num=1,
                                        ),
                                        help_text="A lone item in a row will be displayed full width. More items per row means visually smaller items.",
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "metrics",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "metrics",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "value",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The metric to display, usually a number. For example: 45",
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="The name of the metric. For example: areas mapped",
                                                        required=True,
                                                    ),
                                                ),
                                            ]
                                        ),
                                        max_num=4,
                                        min_num=1,
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "team_carousel",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[], max_length=400, required=True
                                    ),
                                ),
                                (
                                    "team",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.PersonPage"]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "html",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        help_text="(Internal use only.) Explain what this does for other editors. Not displayed on the website.",
                                        required=False,
                                    ),
                                ),
                                ("html", wagtail.blocks.CharBlock(max_length=10000)),
                            ]
                        ),
                    ),
                    (
                        "heading_and_subheading",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        max_length=75, required=True
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=["italic", "bold", "link"],
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "partner_logos",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "partners",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "logo",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        required=True
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "title_text_image",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="The title of the block",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.CharBlock(
                                        help_text="A description displayed under the title",
                                        required=True,
                                    ),
                                ),
                                (
                                    "image",
                                    wagtail.images.blocks.ImageChooserBlock(
                                        help_text="An image, displayed on the left or right of the title and description",
                                        required=True,
                                    ),
                                ),
                                (
                                    "links",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                ),
                                (
                                    "layout",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            ("image_right", "Image right"),
                                            ("image_left", "Image left"),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("impact_area_carousel", wagtail.blocks.StructBlock([])),
                    (
                        "latest_articles",
                        wagtail.blocks.StructBlock(
                            [("title", wagtail.blocks.CharBlock(required=True))]
                        ),
                    ),
                    (
                        "featured_projects",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "ProjectsChooser",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ProjectPage"]
                                        ),
                                        help_text="Projects to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of projects",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "latest_opportunities",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "opportunities_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all opportunities across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show opportunities that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all opportunites across the HOT site, or only those opportunities that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    ("map", wagtail.blocks.StructBlock([])),
                    (
                        "testimonials_slider_block",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "testimonials",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "quote",
                                                    wagtail.blocks.RichTextBlock(
                                                        features=[],
                                                        max_length=400,
                                                        required=True,
                                                    ),
                                                ),
                                                (
                                                    "image",
                                                    wagtail.images.blocks.ImageChooserBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "name",
                                                    wagtail.blocks.CharBlock(
                                                        required=True
                                                    ),
                                                ),
                                                (
                                                    "location",
                                                    wagtail.blocks.CharBlock(
                                                        required=False
                                                    ),
                                                ),
                                            ]
                                        )
                                    ),
                                )
                            ]
                        ),
                    ),
                    (
                        "resources",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "first_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "second_highlighted_resource",
                                    wagtail.blocks.PageChooserBlock(),
                                ),
                                (
                                    "popular_resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.StructBlock(
                                            [
                                                (
                                                    "page",
                                                    wagtail.blocks.PageChooserBlock(
                                                        required=False
                                                    ),
                                                ),
                                                (
                                                    "label",
                                                    wagtail.blocks.CharBlock(
                                                        help_text="Will override the page title if specified",
                                                        max_length=250,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "url",
                                                    wagtail.blocks.URLBlock(
                                                        help_text="Will override the page URL if specified",
                                                        max_length=1000,
                                                        required=False,
                                                    ),
                                                ),
                                                (
                                                    "description",
                                                    wagtail.blocks.CharBlock(
                                                        max_length=250, required=False
                                                    ),
                                                ),
                                            ]
                                        ),
                                        help_text="Popular resources to highlight",
                                        min=1,
                                    ),
                                ),
                                (
                                    "view_all_link",
                                    wagtail.blocks.StructBlock(
                                        [
                                            (
                                                "page",
                                                wagtail.blocks.PageChooserBlock(
                                                    required=False
                                                ),
                                            ),
                                            (
                                                "label",
                                                wagtail.blocks.CharBlock(
                                                    help_text="Will override the page title if specified",
                                                    max_length=250,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "url",
                                                wagtail.blocks.URLBlock(
                                                    help_text="Will override the page URL if specified",
                                                    max_length=1000,
                                                    required=False,
                                                ),
                                            ),
                                            (
                                                "description",
                                                wagtail.blocks.CharBlock(
                                                    max_length=250, required=False
                                                ),
                                            ),
                                        ]
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "upcoming_events",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "events_shown",
                                    wagtail.blocks.ChoiceBlock(
                                        choices=[
                                            (
                                                "show_all",
                                                "Show all events across the site",
                                            ),
                                            (
                                                "only_children",
                                                "Only show events that are childen of this page",
                                            ),
                                        ],
                                        help_text="This block can show all events across the HOT site, or only those events that are under this page.",
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "openmappinghub",
                        wagtail.blocks.StructBlock(
                            [
                                (
                                    "title",
                                    wagtail.blocks.CharBlock(
                                        help_text="Title of the block, typically Open Mapping Hubs.",
                                        required=True,
                                    ),
                                ),
                                (
                                    "description",
                                    wagtail.blocks.RichTextBlock(
                                        features=[],
                                        help_text="Description of the block, found under the title.",
                                        max_length=600,
                                        required=True,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_resources",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "resources",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(),
                                        help_text="Resources to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying resources. Useful for giving context if you are providing a sub-set of resources",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                    (
                        "featured_tools",
                        wagtail.blocks.StructBlock(
                            [
                                ("title", wagtail.blocks.CharBlock(required=True)),
                                (
                                    "tools",
                                    wagtail.blocks.ListBlock(
                                        wagtail.blocks.PageChooserBlock(
                                            page_type=["app.ToolPage"]
                                        ),
                                        help_text="Tools to list in carousel. You can choose as many as you like.",
                                    ),
                                ),
                                (
                                    "sub_title",
                                    wagtail.blocks.CharBlock(
                                        help_text="An optional sub-title to display next to the title when displaying projects. Useful for giving context if you are providing a sub-set of tools",
                                        required=False,
                                    ),
                                ),
                            ]
                        ),
                    ),
                ],
                blank=True,
                null=True,
                use_json_field=True,
            ),
        ),
    ]
